...
int main(int argc, char **argv) {
  ...
  Vec vecx, vecy;
  ierr = VecCreateMPI(PETSC_COMM_WORLD, vec_size_local, vec_size, &vecx);
  CHKERRQ(ierr);
  ierr = VecCreateMPI(PETSC_COMM_WORLD, vec_size_local, vec_size, &vecy);
  CHKERRQ(ierr);
  ...
  //     PetscSynchronizedPrintf(PETSC_COMM_WORLD,"rank is %d; px = %g+%gi; py =
  //     %g+%gi\n", rank, (double)PetscRealPart(px[i]),
  //     (double)PetscImaginaryPart(px[i]), (double)PetscRealPart(py[i]),
  //     (double)PetscImaginaryPart(py[i]));
  // }
  // PetscSynchronizedFlush(PETSC_COMM_WORLD,PETSC_STDOUT);
