...
void mat_insert(Mat A, int low, int high, data& conf, double mu_q, double mass){
    ...
    for(int place = low;place < high;place+=2){
        ...
        for(int mu = 1;mu <= 4;mu++){
            ...
            MatSetValue(A, place, complex_place(link_ferm), (float)(exp(mu_q * delta_4)/2 * border_sign * sign) * ((float)B.a0 + (float)B.a3*PETSC_i), INSERT_VALUES);
            MatSetValue(A, place, complex_place(link_ferm) + 1, (float)(exp(mu_q * delta_4)/2 * border_sign * sign) * ((float)B.a2 + (float)B.a1*PETSC_i), INSERT_VALUES);
            MatSetValue(A, place + 1, complex_place(link_ferm), (float)(exp(mu_q * delta_4)/2 * border_sign * sign) * (-(float)B.a2 + (float)B.a1*PETSC_i), INSERT_VALUES);
            MatSetValue(A, place + 1, complex_place(link_ferm) + 1, (float)(exp(mu_q * delta_4)/2 * border_sign * sign) * ((float)B.a0 - (float)B.a3*PETSC_i), INSERT_VALUES);
            link.move_dir(-mu);
            link_ferm.move(-mu, 1);
            border_sign = link_ferm.border_sign(-mu);
            link_ferm.move(-mu, 1);
