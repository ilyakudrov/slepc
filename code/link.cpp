...
link1::link1(int dir, int lattice_size_x, int lattice_size_y,
             int lattice_size_z, int lattice_size_t) {
  lattice_size[0] = lattice_size_x;
  lattice_size[1] = lattice_size_y;
  lattice_size[2] = lattice_size_z;
  lattice_size[3] = lattice_size_t;
...
link1::link1(int coord[4], int dir, int lattice_size_x, int lattice_size_y,
             int lattice_size_z, int lattice_size_t) {
  lattice_size[0] = lattice_size_x;
  lattice_size[1] = lattice_size_y;
  lattice_size[2] = lattice_size_z;
...
link1::link1(int x, int y, int z, int t, int dir, int lattice_size_x,
             int lattice_size_y, int lattice_size_z, int lattice_size_t) {
  lattice_size[0] = lattice_size_x;
  lattice_size[1] = lattice_size_y;
  lattice_size[2] = lattice_size_z;
  lattice_size[3] = lattice_size_t;
...
link1::link1(int x, int y, int z, int t, int lattice_size_x, int lattice_size_y,
             int lattice_size_z, int lattice_size_t) {
  lattice_size[0] = lattice_size_x;
  lattice_size[1] = lattice_size_y;
  lattice_size[2] = lattice_size_z;
...
int link1::get_place() { return PLACE1_LINK; }

int link1::get_place1() { return PLACE_FIELD_LINK; }
...
matrix link1::plaket(const data &conf) {
  ...
  move(dir, 1);
  move_dir(-4); // the same temporal direction as polyakov loop, connected to
                // schwinger line, has
  A = A * get_matrix(conf.array);
  move(-4, 1);
  move_dir(-dir);
...
double link1::field1(const vector<vector<matrix>> &schwinger_line,
                     const vector<matrix> &plaket,
                     const vector<matrix> &polyakov_loop, int d, int D, int dir,
                     int x) {
  int dir1 = direction;
...
double link1::field2(const vector<matrix> &plaket,
                     const vector<matrix> &polyakov_loop, int d, int D, int dir,
                     int x) {
  int dir1 = direction;
  move_dir(-4);
...
matrix link1::staples_second(const vector<vector<matrix>> &smearing_first,
                             int eta, int nu) {
  matrix A;
  matrix B;
  int dir = direction;
...
matrix link1::staples_second_refresh(const vector<matrix> &vec, int eta, int nu,
                                     double alpha3) {
  matrix A;
  matrix B;
  int dir = direction;
...
matrix link1::staples_third(const vector<vector<matrix>> &smearing_second,
                            int eta) {
  matrix A;
  matrix B;
  int dir = direction;
...
matrix link1::staples_third_refresh(const vector<matrix> &vec, int eta,
...
vector<matrix> link1::smearing_first(const data &conf, double alpha3, int nu,
                                     int rho) {
...
vector<vector<matrix>> link1::smearing_first_full(const data &conf,
                                                  double alpha3) {
...
vector<matrix> link1::smearing_second(const data &conf,
                                      vector<vector<matrix>> &smearing_first,
                                      double alpha2, int nu) {
  vector<matrix> vec(data_size);
  matrix A;
  for (int t = 0; t < t_size; t++) {
    for (int z = 0; z < z_size; z++) {
...
vector<vector<matrix>> link1::smearing_second_full(
    const data &conf, vector<vector<matrix>> &smearing_first, double alpha2) {
...
vector<matrix> link1::smearing_HYP(data &conf,
...
matrix link1::smearing_first_refresh(const vector<matrix> &vec, int nu, int rho,
                                     double alpha3) {
  matrix A;
...
matrix link1::smearing_second_refresh(const vector<matrix> &vec, int nu,
                                      double alpha2, double alpha3) {
  matrix A;
...
vector<matrix> link1::smearing_HYP_refresh(data &conf, double alpha1,
                                           double alpha2, double alpha3) {
  vector<matrix> vec(data_size);
  vec = conf.array;
  matrix A;
...
double link1::get_current(data &conf) {
  ...
  Lt.move(4, 1);
  if (direction == 4)
    jj = Lx.T(conf, 2, 3) - T(conf, 2, 3) - (Ly.T(conf, 1, 3) - T(conf, 1, 3)) +
         Lz.T(conf, 1, 2) - T(conf, 1, 2);
  if (direction == 1)
    jj = -(Lt.T(conf, 2, 3) - T(conf, 2, 3)) +
         (Ly.T(conf, 4, 3) - T(conf, 4, 3)) -
         (Lz.T(conf, 4, 2) - T(conf, 4, 2));
  if (direction == 2)
    jj = Lt.T(conf, 1, 3) - T(conf, 1, 3) - (Lx.T(conf, 4, 3) - T(conf, 4, 3)) +
         Lz.T(conf, 4, 1) - T(conf, 4, 1);
  if (direction == 3)
    jj = -(Lt.T(conf, 1, 2) - T(conf, 1, 2)) +
         (Lx.T(conf, 4, 2) - T(conf, 4, 2)) -
         (Ly.T(conf, 4, 1) - T(conf, 4, 1));
  return jj / 2. / Pi;
}
