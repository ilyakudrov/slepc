...
public:
  ...
  link1(int lattice_size_x, int lattice_size_y, int lattice_size_z,
        int lattice_size_t);
  link1(int dir, int lattice_size_x, int lattice_size_y, int lattice_size_z,
        int lattice_size_t);
  link1(int coord[4], int dir, int lattice_size_x, int lattice_size_y,
        int lattice_size_z, int lattice_size_t);
  link1(int x, int y, int z, int t, int dir, int lattice_size_x,
        int lattice_size_y, int lattice_size_z, int lattice_size_t);
  link1(int x, int y, int z, int t, int lattice_size_x, int lattice_size_y,
        int lattice_size_z, int lattice_size_t);
  ...
  int get_place1();
  matrix
  get_matrix(const vector<matrix>
                 &vec); // works with negative directions(takes inverse matrix)
  matrix get_matrix1(matrix *vec);
  double border_sign(int mu);
  double get_angle_abelian(const vector<matrix> &vec);
  matrix schwinger_line(
      const data &conf, int d, int dir,
  ...
  matrix polyakov_loop(const data &conf); // attached to where it loops and
  ...
  double field1(
      const vector<vector<matrix>> &schwinger_line,
      const vector<matrix> &plaket, const vector<matrix> &polyakov_loop, int d,
      int D, int dir,
      int x); // Link is attached to the "left" source, dir points to the plaket
  // second numerator
  double
  field2(const vector<matrix> &plaket, const vector<matrix> &polyakov_loop,
         int d, int D, int dir,
         int x); // attached to the "left" source, dir points to the plaket
  // denominator
  double field3(const vector<matrix> &polyakov_loop, int D,
                int x); // attached to the "left" source and points to another
  matrix staples_first(const vector<matrix> &vec, int eta);
  matrix staples_second(const vector<vector<matrix>> &smearing_first, int eta,
                        int nu);
  matrix
  staples_second_refresh(const vector<matrix> &vec, int eta, int nu,
  ...
  matrix staples_third_refresh(const vector<matrix> &vec, int eta,
                               double alpha2, double alpha3);
  vector<matrix> smearing_first(const data &conf, double alpha3, int nu,
                                int rho);
  vector<vector<matrix>> smearing_first_full(const data &conf, double alpha3);
  vector<matrix> smearing_second(const data &conf,
                                 vector<vector<matrix>> &smearing_first,
                                 double alpha2, int nu);
  vector<vector<matrix>>
  smearing_second_full(const data &conf, vector<vector<matrix>> &smearing_first,
                       double alpha2);
  vector<matrix> smearing_HYP(data &conf,
  ...
  matrix smearing_first_refresh(const vector<matrix> &vec, int nu, int rho,
                                double alpha3); // refresh link every step
  matrix smearing_second_refresh(const vector<matrix> &vec, int nu,
                                 double alpha2,
                                 double alpha3); // refresh link every step
  vector<matrix> smearing_HYP_refresh(data &conf, double alpha1, double alpha2,
                                      double alpha3); // refresh link every step
  vector<matrix>
  smearing_APE_refresh(data &conf, double alpha_APE); // refresh link every step
