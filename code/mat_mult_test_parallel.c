...
int main(int argc, char **argv) {
  ...
  my_data.conf.read_float("/home/ilya/lattice/slepc/conf/nosmeared/time_32/"
                          "mu0.10/conf_0001.fl" /*argv[5]*/);
  ...
  ierr = VecCreateMPI(comm, vec_size, vec_size, &vecx);
  CHKERRQ(ierr);
  ierr = VecCreateMPI(comm, vec_size_local, vec_size, &vecy);
  CHKERRQ(ierr);
...
void MatVecMult(const matrix &A, const PetscScalar *x, PetscScalar *y,
                int border_sign) {
...
PetscErrorCode MatMult_test_parallel(const Vec vecx, Vec vecy,
                                     const mat_data ctx) {
  ...
  for (int i = 0; i < local_size / 2; i++) {
    ...
    for (int mu = 1; mu <= 4; mu++) {
      ...
      // link.get_matrix1(data_conf), &src[complex_place(link_ferm)], &vec,
      // src_index, border_sign);
      MatVecMult(exp(mu_q * delta_4) / 2 * sign * link.get_matrix(conf.array),
...
PetscErrorCode create_test_vec(Vec vecx) {
  ...
  for (int i = 0; i < local_size / 2; i++) {
    ...
    int z = (place - (x_size * y_size * z_size * 2) * (t - 1)) /
                (x_size * y_size * 2) +
            1;
    int y = (place - (x_size * y_size * z_size * 2) * (t - 1) -
             (2 * x_size * y_size) * (z - 1)) /
                (x_size * 2) +
            1;
    int x = (place - (x_size * y_size * z_size * 2) * (t - 1) -
             (2 * x_size * y_size) * (z - 1) - 2 * x_size * (y - 1)) /
                2 +
            1;
    px[i * 2] =
